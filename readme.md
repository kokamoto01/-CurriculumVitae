...
- 退職済みの前担当者が残した書類に基付いて、基本設計書の再構成を行った
- CI/CD化(CodePipeline, CodeBuild, CloudFormation)を見据えたインフラ設計・構築
...
- 技術リーダーとしての責任の重さからメンタルを保つための工夫を考えるようになった
  - 「ザッソウ」を意識して、息苦しさの解消を心がけた

【環境】
- Docker
  - nginx + php-fpm(CakePHP4)
  - nginx + OpenResty
- CloudFormation
  - ECS(EC2, Fargate), Aurora, ElastiCache, ELB, Route53, S3
